...
class TestVowels(unittest.TestCase):
    def test_vowel_a_1(self):  # Ударный после твёрдого согласного
        ...
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['t', 'r', 'ɐ', 'v', 'a'], ru_transcript.get_allophones())

    def test_vowel_a_2(self):  # Ударный после твёрдого согласного перед 'л'
        testing_text = 'палка'
        testing_a_text = 'па+лка'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['p', 'ɑ', 'ɫ', 'k', 'ʌ'], ru_transcript.get_allophones())

    def test_vowel_a_3(self):  # Ударный не после твёрдого согласного
        testing_text = 'пять'
    ...
    def test_vowel_a_5(self):  # Предударный после твёрдого согласного или в начале слова
        testing_text = 'паром'
        testing_a_text = 'паро+м'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['p', 'ɐ', 'rʷ', 'o', 'm'], ru_transcript.get_allophones())

    def test_vowel_a_6(self):  # Предударный не после твёрдого согласного
    ...
    def test_vowel_a_8(self):  # II предударный или заударный после твёрдого согласного или в начале слова
        testing_text = 'акварель'
        testing_a_text = 'акваре+ль'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['ə', 'k', 'v', 'ɐ', 'rʲ', 'e', 'lʲ'], ru_transcript.get_allophones())

    def test_vowel_a_9(self):  # II предударный или заударный после твёрдого согласного в финальном слоге
        testing_text = 'собака'
        testing_a_text = 'соба+ка'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['s', 'ɐ', 'b', 'a', 'k', 'ʌ'], ru_transcript.get_allophones())

    def test_vowel_a_10(self):  # II предударный или заударный не после твёрдого согласного не в окончании
        testing_text = 'тяжеленный'
        testing_a_text = 'тяжеле+нный'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['tʲ', 'ɪ.', 'ʐ', 'ə', 'lʲ', 'e', 'nːˠ', 'ᵻ', 'j'], ru_transcript.get_allophones())

    def test_vowel_a_11(self):  # II предударный или заударный не после твёрдого согласного (только в окончании)
        testing_text = 'гуляя'
    ...
    def test_vowel_o_1(self):  # Ударный после твёрдого согласного или в начале слова
        testing_text = 'облако'
        testing_a_text = 'о+блако'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['o', 'b', 'ɫ', 'ə', 'k', 'ʌ'], ru_transcript.get_allophones())

    def test_vowel_o_2(self):  # Ударный не после твёрдого согласного
        testing_text = 'тётя'
    ...
    def test_vowel_o_4(self):  # Предударный после твёрдого согласного или в начале слова
        testing_text = 'стопа'
        testing_a_text = 'стопа+'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['s', 't', 'ɐ', 'p', 'a'], ru_transcript.get_allophones())

    def test_vowel_o_5(self):  # Предударный не после твёрдого согласного
    ...
    def test_vowel_o_7(self):  # II предударный или заударный после твёрдого согласного или в начале слога
        testing_text = 'молоко'
        testing_a_text = 'молоко+'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['m', 'ə', 'ɫ', 'ɐ', 'kʷ', 'o'], ru_transcript.get_allophones())

    def test_vowel_o_8(self):  # II предударный или заударный после твёрдого согласного в финальном слоге
        testing_text = 'озеро'
        testing_a_text = 'о+зеро'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['o', 'zʲ', 'ɪ.', 'r', 'ʌ'], ru_transcript.get_allophones())

    def test_vowel_o_9(self):  # II предударный или заударный не после твёрдого согласного
        testing_text = 'огайо'
    ...
    def test_vowel_o_10(self):  # II предударный или заударный после шипящих и 'ц'
        ...
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['ʂ', 'ə', 'k', 'ɐ', 'ɫ', 'a', 't'], ru_transcript.get_allophones())

    def test_vowel_e_1(self):  # Ударный после твёрдого согласного или в начале слова
        testing_text = 'это'
        testing_a_text = 'э+то'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['ɛ', 't', 'ʌ'], ru_transcript.get_allophones())

    def test_vowel_e_2(self):  # Ударный не после твёрдого согласного
        testing_text = 'пень'
    ...
    def test_vowel_e_4(self):  # Предударный после твёрдого согласного или в начале слова (ыэ)
        testing_text = 'этап'
        testing_a_text = 'эта+п'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['ᵻ', 't', 'a', 'p'], ru_transcript.get_allophones())

    def test_vowel_e_5(self):  # Предударный не после твёрдого согласного и не в начале слова
        testing_text = 'велюр'
        testing_a_text = 'велю+р'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['vʲ', 'ɪ', 'lᶣ', 'ʉ', 'r̥'], ru_transcript.get_allophones())

    def test_vowel_e_6(self):  # II предударный или заударный не после твёрдого согласного или в начале слова
        testing_text = 'пепел'
        testing_a_text = 'пе+пел'
        ru_transcript = transcribe(testing_text, testing_a_text)
        self.assertEqual(['pʲ', 'e', 'pʲ', 'ɪ.', 'ɫ'], ru_transcript.get_allophones())
    ...
    def test_vowel_u_1(self):  # Ударный после твёрдого согласного
        testing_text = 'пуля'
    ...
    def test_vowel_ii_1(self):  # Ударный после твёрдого согласного
        testing_text = 'ты'
        testing_a_text = 'ты+'
        ru_transcript = transcribe(testing_text, testing_a_text)
